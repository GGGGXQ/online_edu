...
class AliPaySDK(AliPay):
    ...
    def page_pay(self, order_number, real_price, order_name):
        """
        生成支付链接
        @params order_number: 商户订单号
        @params real_price: 订单金额
        @params order_name: 订单标题
        @return 支付链接
        """
        order_string = self.client_api(
            ...
            return_url=self.config["return_url"],  # 可选，同步回调地址，必须填写客户端地址
            notify_url=self.config["notify_url"]  # 可选，不填则采用全局默认notify_url，必须填写服务端的路径
        )
