...
class CourseDirection(BaseModel):
    ...
    recomment_home_top = models.BooleanField(default=False, verbose_name="是否推荐到首页首页必学栏目")

    class Meta:
        db_table = "ol_course_direction"
        verbose_name = "学习方向"
...
class Course(BaseModel):
    ...
    attachment_path = models.FileField(max_length=1000, blank=True, null=True, verbose_name="课程路径")
    ...
    recomment_home_hot = models.BooleanField(default=False, verbose_name="是否推荐到首页新课栏目")
    recomment_home_top = models.BooleanField(default=False, verbose_name="是否推荐到首页首页必学栏目")
    direction = models.ForeignKey("CourseDirection", related_name='course_list', on_delete=models.DO_NOTHING,
    ...
    def course_cover_small(self):
        if self.course_cover:
            return format_html(f'<img style="border-radius: 0%;" src="{self.course_cover.thumb_108x61.url}">')
        return ""
    ...
    def course_cover_medium(self):
        if self.course_cover:
            return format_html(f'<img style="border-radius: 0%;" src="{self.course_cover.thumb_540x304.url}">')
        return ""
    ...
    def course_cover_large(self):
        if self.course_cover:
            return format_html(f'<img style="border-radius: 0%;" src="{self.course_cover.thumb_1080x608.url}">')
        return ""
    ...
    def discount(self):
        ...
        if last_activity_log:
            # 获取优惠类型的提示文本
            type_text = last_activity_log.activity.discount.discount_type.name
            # 获取限时活动剩余时间戳[单位：s]
...
class Teacher(BaseModel):
    ...
    role = models.SmallIntegerField(choices=role_choice, default=0, verbose_name="讲师身份")
    ...
    def avatar_medium(self):
        if self.avatar:
            return format_html(f'<img style="border-radius: 0%;"src="{self.avatar.thumb_400x400.url}">')
        return ""
    ...
    def avatar_large(self):
        if self.avatar:
            return format_html(f'<img style="border-radius: 0%;"src="{self.avatar.thumb_800x800.url}">')
        return ""
...
class CourseChapter(BaseModel):
    ...
    summary = RichTextField(blank=True, null=True, verbose_name="章节介绍")
    pub_date = models.DateField(auto_now_add=True, verbose_name="发布日期")
    course = models.ForeignKey("Course", related_name='chapter_list', on_delete=models.CASCADE,
                               db_constraint=False, verbose_name="课程名称")

    class Meta:
        db_table = "ol_course_chapter"
        verbose_name = "课程章节"
        verbose_name_plural = verbose_name
...
class CourseLesson(BaseModel):
    ...
    lesson_type = models.SmallIntegerField(choices=lesson_type_choices, default=2, verbose_name="课时种类")
    lesson_link = models.CharField(max_length=255, blank=True, null=True,
                                   help_text="若是video，填视频地址或者视频id，若是文档，填文档地址",
                                   verbose_name="课时链接")
    duration = models.CharField(blank=True, null=True, max_length=32, verbose_name="课时时长")
    ...
    chapter = models.ForeignKey("CourseChapter", related_name='lesson_list', on_delete=models.CASCADE,
                                db_constraint=False, verbose_name="章节名称")
    course = models.ForeignKey("Course", related_name='lesson_list', on_delete=models.DO_NOTHING,
                               db_constraint=False, verbose_name="课程")

    class Meta:
        db_table = "ol_course_lesson"
        verbose_name = "课程课时"
        verbose_name_plural = verbose_name
...
class Discount(BaseModel):
    discount_type = models.ForeignKey("DiscountType", related_name='discount_list', on_delete=models.CASCADE,
                                      db_constraint=False, verbose_name="优惠类型")
    condition = models.IntegerField(blank=True, default=0, verbose_name="满足优惠的条件", help_text="设置享受优惠的价格条件,如果不填或0则没有优惠门槛")
...
class CourseActivityPrice(BaseModel):
    ...
    course = models.ForeignKey("Course", related_name='price_list', on_delete=models.CASCADE,
                               db_constraint=False, verbose_name="课程")
    discount = models.ForeignKey("Discount", related_name='price_list', on_delete=models.CASCADE,
