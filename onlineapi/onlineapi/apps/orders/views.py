...
class OrderListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        order_status = order_status = int(self.request.query_params.get("status", -1))
...
class OrderViewSet(ViewSet):
    ...
    def pay_cancel(self, request, pk):
        ...
        try:
            ...
            return Response({"message": "当前订单不存在或不能取消！"}, status=status.HTTP_400_BAD_REQUEST)

        with transaction.atomic():
            save_id = transaction.savepoint()
            try:
                # 1.查询当前订单是否使用了积分，如有则恢复
                if order.credit > 0:
                    order.user.credit += order.credit
                    order.user.save()
                # 2.查询当前订单是否使用了优惠券，如有则恢复
                ...
                return Response({"message": "当前订单无法取消，请联系客服工作人员！"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
